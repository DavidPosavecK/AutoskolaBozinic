...
function renderReviews() {
  ...
  reviewsList.innerHTML = googleReviews.map(review => `
    <div class="p-6 rounded-xl bg-slate-800/50 border border-slate-700 hover:border-amber-500/30 transition">
      <div class="flex justify-between items-start gap-4 mb-3">
        <div>
          <div class="font-bold text-slate-100">${review.name}</div>
          <div class="text-slate-500 text-sm">${review.date}</div>
        </div>
        <div class="text-amber-400 text-lg">${generateStars(review.rating)}</div>
      </div>
      <div class="text-slate-400 text-sm leading-relaxed">${review.text}</div>
    </div>
  `).join('');
}
